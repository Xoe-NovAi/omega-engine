...
On-demand invocation available via: omega summon Roc Racoon "mining brief"
"""
...
# Resolve project root relative to this file (src/omega/entity_roc_racoon.py -> project root)
...
class RocRacoonMiner:
    ...
    def submit_to_queue(self, artifacts: List[Dict[str, Any]]):
        ...
        logger.info(f"Roc Racoon: Added {new_count} new artifacts to mining queue (total: {len(self.mining_queue)})")
    ...
    async def deep_analyze(self, gemma_api_func) -> str:
        ...
        This is called by the Roc Racoon Quadlet service.
        ...
        for art in artifacts:
            ...
            if fpath.exists():
                    try:
                        content = await anyio.Path(fpath).read_text(errors='replace')
                ...
                if fpath2.exists():
                    try:
                        content = await anyio.to_thread.run_sync(lambda: fpath2.read_text(errors='replace')[:2000])
                    except (IOError, UnicodeDecodeError):
                        content = "[binary or unreadable]"
                else:
        ...
        prompt = (
            "You are Roc Racoon, P0 Legacy Deep Mining Keeper of the Omega Engine.\n"
