...
Your domain covers:
- Background researcher (including the **Roc Racoon** legacy mining persona and `roracoon-3b` model)
...
- **Verify the Firewall Fixes.** The core engine recently underwent a hardening sprint. Verify that `src/omega/workers/background_researcher/soul_updater.py` and `distiller.py` no longer contain hardcoded entity mappings or stack-specific prompts. They must use the `EntityRegistry` and `config/distiller_prompts.yaml`.
