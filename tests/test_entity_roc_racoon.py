"""Tests for Roc Racoon Legacy Deep Mining Keeper.
...
class TestRocRacoonMinerInit:
    def test_init_loads_empty_history(self, roc_racoon):
        """RocRacoonMiner initializes with an empty mining queue."""
        assert roc_racoon.mining_queue == []
        assert len(roc_racoon.mines) > 0

    def test_init_loads_existing_history(self, roc_racoon, mock_data_dir):
        """RocRacoonMiner loads mining history JSON if it exists."""
    ...
    def test_init_invalid_history(self, roc_racoon, mock_data_dir):
        """RocRacoonMiner handles corrupt history JSON gracefully."""
        history_dir = mock_data_dir / "mining_queue"
        history_dir.mkdir(parents=True, exist_ok=True)
...
class TestScanMine:
    def test_scan_mine_finds_files(self, roc_racoon, temp_mine):
        """scan_mine should find .yaml, .md, .py, .json files."""
        artifacts = roc_racoon.scan_mine("test-mine", temp_mine)
    ...
    def test_scan_mine_nonexistent(self, roc_racoon):
        """scan_mine on nonexistent path returns empty list."""
        artifacts = roc_racoon.scan_mine("void", Path("/tmp/nonexistent_omega_mine_xyz"))
        assert artifacts == []

    def test_scan_mine_skips_large_files(self, roc_racoon, temp_mine):
    ...
    def test_scan_mine_has_metadata(self, roc_racoon, temp_mine):
...
class TestClassifyArtifact:
    @pytest.fixture
    def roc_racoon_classify(self, roc_racoon):
        return roc_racoon

    def test_classify_config_yaml(self, roc_racoon):
        """Config .yaml files are 'technical'."""
        a = {"path": "project/config.yaml", "suffix": ".yaml"}
        assert roc_racoon.classify_artifact(a) == "technical"

    def test_classify_config_yml(self, roc_racoon):
        """Config .yml files are 'technical'."""
        a = {"path": "project/settings.yml", "suffix": ".yml"}
        assert roc_racoon.classify_artifact(a) == "technical"

    def test_classify_session(self, roc_racoon):
        """Session/chat files are 'strategic'."""
        a = {"path": "sessions/chat_export.md", "suffix": ".md"}
        assert roc_racoon.classify_artifact(a) == "strategic"

    def test_classify_python(self, roc_racoon):
        """Python source files are 'technical'."""
        a = {"path": "src/module.py", "suffix": ".py"}
        assert roc_racoon.classify_artifact(a) == "technical"

    def test_classify_markdown(self, roc_racoon):
        """Non-session markdown files are 'archival'."""
        a = {"path": "docs/notes.md", "suffix": ".md"}
        assert roc_racoon.classify_artifact(a) == "archival"

    def test_classify_noise(self, roc_racoon):
        """Unknown extensions are 'noise'."""
        a = {"path": "file.txt", "suffix": ".txt"}
        assert roc_racoon.classify_artifact(a) == "noise"

    def test_classify_json(self, roc_racoon):
...
class TestSubmitToQueue:
    def test_submit_adds_artifacts(self, roc_racoon, temp_mine):
        """submit_to_queue should classify and add new artifacts."""
        artifacts = roc_racoon.scan_mine("test-mine", temp_mine)
        roc_racoon.submit_to_queue(artifacts)
        assert len(roc_racoon.mining_queue) == len(artifacts)
        # All should have classification assigned
        for a in roc_racoon.mining_queue:
            assert "classification" in a

    def test_submit_deduplicates(self, roc_racoon, temp_mine):
    ...
    def test_submit_persists_to_disk(self, roc_racoon, temp_mine, mock_data_dir):
